(function () {
  ...
  var take = function (limit) {
    return function (source) {
      return new Observable(function (observer) {
        var i = 0;
        return source.subscribe({
          next: function (x) {
            i++; observer.next(x); if (i == limit) {
              observer.complete();
            }
          },
          error: function (err) { return observer.error(err); },
  ...
  var withLatestFrom = function (right) {
    return function (source) {
      return new Observable(function (observer) {
        var right_val = null;
        var sourceObserver = {
  ...
  var fromEvent = function (element, eventname) {
    return new Observable(function (observer) {
  ...
  var orientation$ = pipe(fromEvent(window, 'deviceorientation'), map(function (event) { return ({ latest: { alpha: event.alpha, beta: event.beta, gamma: event.gamma }, count: 1, distance: 0 }); }), startWith({ latest: null, count: 0, distance: 0 }), scan(function (acc, a) {
    return ({
